...
def trades_loss(model,normalize, x, y,y_t, eps, beta, alpha, n_iters=10):
    x_natural = x
    cw_beta = beta
...
def cw_trades_loss(model,normalize, x, y, y_t,cw_eps,beta, alpha, attack_mode_UT, n_iters=10):   
    x_natural = x
    cw_beta = beta
    eps = cw_eps
    perturb_steps = n_iters
    ...
    for _ in range(perturb_steps):
        ...
        with torch.enable_grad():
            if attack_mode_UT:
                ...
                loss = F.cross_entropy(model(normalize_cifar(x_adv)), y_t)
...
def fat_loss(model,normalize, x, y,y_t, eps, beta, alpha, n_iters=10):
    tau = beta 
    normalize_cifar = normalize
    K = n_iters
...
def cw_fat_loss(model,normalize,x, y,y_t,cw_eps, beta, alpha,attack_mode_UT, n_iters=10):
    normalize_cifar = normalize 
    tau = beta
    batch_eps = cw_eps[y]
    delta = torch.zeros_like(x).to(x.device)
